...
const AdminFAQ = () => {
  ...
  const fetchQuestions = async () => {
    setLoading(true);
    try {
      const { data } = await supabase.rpc("get_question_stats", { limit_count: 20 });
      if (data) setQuestions(data as unknown as QuestionStat[]);
    } catch (e) {
  ...
  const totalCount = questions.reduce((sum, q) => sum + Number(q.count), 0);
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { value: questions.length, label: "سؤال فريد" },
          { value: totalCount.toLocaleString("ar-SA"), label: "إجمالي التكرارات" },
          { value: questions.length > 0 ? questions[0].count : 0, label: "أكثر سؤال تكراراً" },
        ].map((s, i) => (
          <Card key={i} className={`transition-all duration-300 animate-fade-in-up hover:translate-y-[-2px] ${isDark ? "glass-card border-0" : ""}`} style={{ animationDelay: `${0.1 + i * 0.08}s`, opacity: 0 }}>
            <CardContent className="p-5 text-center">
              <p className={`text-2xl font-bold ${i === 2 ? "text-primary" : "text-foreground"}`}>{s.value}</p>
        ...
        <CardContent>
          {questions.length === 0 ? (
            <p className="text-center text-muted-foreground py-8 text-sm">
              لم تُسجَّل أي أسئلة بعد. ستظهر هنا تلقائياً عند بدء استخدام المحادثة.
            </p>
          ) : (
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        آخر سؤال: {new Date(q.last_asked).toLocaleDateString("ar-SA")}
                      </p>
                    </div>
                  </div>
