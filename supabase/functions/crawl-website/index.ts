...
serve(async (req) => {
  ...
  try {
    ...
    for (const page of pages) {
      ...
      try {
        ...
        if (existing && existing.content_hash === hash) {
          // Touch last_crawled_at and continue
          await supabase
            .from("knowledge_documents")
        ...
        // Process via existing pipeline (chunks + embeddings)
    ...
    return new Response(
      JSON.stringify({ success: true, added, updated, skipped, failed, total: pages.length }),
