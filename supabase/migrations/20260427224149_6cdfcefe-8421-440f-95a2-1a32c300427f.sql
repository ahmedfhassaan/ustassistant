...
DECLARE
  ...
  wt double precision;
  ws double precision;
  -- Common Arabic stop / connector words to ignore in keyword scoring
...
BEGIN
  ...
  FROM scored s
  WHERE
    -- Drop chunks that match neither a meaningful keyword nor a semantic threshold
    (s.keyword_score > 0 OR s.fts_score > 0 OR s.sem_score > 0.5)
  ORDER BY rank DESC
  LIMIT max_results;
END;
$function$;